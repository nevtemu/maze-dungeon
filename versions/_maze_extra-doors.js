...
function getRandom(min, max) {
    ...
    function createExtraPassages (times){
        for (let i=1; i<=times; i++){
            let newDoor = passable[getRandom(0, passable.length -1)];
            ...
            let l = document.getElementById(`${y}.${x-1}`);
            let r = document.getElementById(`${y}.${x+1}`);
            if (u){ if(passable.includes(u.id)){ways.push('u');}}
            if (d){ if(passable.includes(d.id)){ways.push('d');}}
            if (l){ if(passable.includes(l.id)){ways.push('l');}}
            if (r){ if(passable.includes(r.id)){ways.push('r');}}
            if (ways.length >= 1){
                let direction = ways[getRandom(0, ways.length-1)]
                switch (direction){
                    case 'u':
    ...
    createExtraPassages(EXTRA_PASSAGE)